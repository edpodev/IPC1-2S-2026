...
Los tipos primitivos son ocho, y guardan el valor directo en memoria (más abajo se ve exactamente dónde). Se dividen en:

- **Enteros**: `byte`, `short`, `int`, `long`. La diferencia entre ellos es solo cuánto espacio reservan (el segundo diagrama de abajo lo muestra a escala). En la mayoría de los casos se usa `int`; `long` aparece cuando el número se pasa de los ~2100 millones (por ejemplo, contando milisegundos desde 1970).
...
![Variables primitivas copiadas por valor en el stack, junto a un arreglo copiado por referencia en el heap](../assets/stack-heap-valor-referencia.svg)

![Espacio en memoria de cada tipo primitivo numérico, a escala: byte, short, char, int, float, long y double](../assets/tipos-primitivos-tamanos.svg)
